...
const register = async (req, res) => {
    try {
        ...
        res.cookie('coderCookie', token, cookieOptions).json({ 
            status: "success", 
    ...
    try {
        ...
        if (!isValidPassword) {
            ...
            return res.status(401).json({ 
                ...
                attemptsLeft: 5 - (user.failedLoginAttempts + 1)
            });
        }
...
const current = async (req, res) => {
    try {
        ...
        if (user.email) {
            ...
            if (dbUser) {
                await usersService.updateUserLastConnection(dbUser._id);
            }
        }
        
        return res.json({ 
            status: "success", 
            user: UserDTO.getUserProfileFrom(user)
        });
...
const logout = async (req, res) => {
    try {
        ...
        res.clearCookie('coderCookie', cookieOptions).json({ 
            status: "success", 
        ...
        res.clearCookie('coderCookie').status(500).json({ 
            status: "error", 
