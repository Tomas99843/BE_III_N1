...
const getAllAdoptions = async(req, res) => {
    try {
        ...
        if (!result.results || result.results.length === 0) {
            req.logger.info('No se encontraron adopciones');
        } else {
            req.logger.info(`Se encontraron ${result.results.length} adopciones`);
        }
    ...
    try {
        ...
        if (!isAdmin && !isOwner && !isPetOwner) {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para ver esta adopción"
            });
        }
...
const updateAdoption = async(req, res) => {
    try {
        ...
        if (updateData.status && !['pending', 'approved', 'rejected', 'completed'].includes(updateData.status)) {
            return res.status(400).json({
                status: "error",
                error: 'Estado inválido. Estados permitidos: pending, approved, rejected, completed'
            });
        }
        ...
        if ((updateData.status === 'approved' || updateData.status === 'rejected') && !isAdmin) {
            return res.status(403).json({
                status: "error",
                error: "Solo administradores pueden aprobar o rechazar adopciones"
            });
        }
...
const deleteAdoption = async(req, res) => {
    try {
        ...
        if (!['pending', 'rejected'].includes(adoption.status)) {
            return res.status(400).json({
                status: "error",
                error: `No se puede eliminar una adopción con estado: ${adoption.status}`
            });
        }
        ...
        res.json({
            ...
            data: { id: adoptionId, deletedAt: new Date() }
        });
...
const createAdoption = async(req, res) => {
    try {
        ...
        if (req.user?.id !== uid) {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para crear adopción para otro usuario"
            });
        }
...
const getUserAdoptions = async(req, res) => {
    try {
        ...
        if (requestingUserId !== userId && req.user?.role !== 'admin') {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para ver estas adopciones"
            });
        }
...
const approveAdoption = async (req, res) => {
    try {
        ...
        if (req.user?.role !== 'admin') {
            return res.status(403).json({
                status: "error",
                error: "Solo administradores pueden aprobar adopciones"
            });
        }
...
const rejectAdoption = async (req, res) => {
    try {
        ...
        if (req.user?.role !== 'admin') {
            return res.status(403).json({
                status: "error",
                error: "Solo administradores pueden rechazar adopciones"
            });
        }
...
const cancelAdoption = async (req, res) => {
    try {
        ...
        if (!isOwner && !isAdmin) {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para cancelar esta adopción"
            });
        }
