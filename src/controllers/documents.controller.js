...
class DocumentsController {
    uploadDocuments = async (req, res) => {
        try {
            ...
            if (user.documents.length + files.length > maxDocuments)
                return res.status(400).json({ status: 'error', message: `No se pueden subir más de ${maxDocuments} documentos. Actual: ${user.documents.length}, Intento: ${files.length}` });
            
            const uploadedDocuments = files.map(file => {
                const sanitizedName = sanitizeInput(file.originalname);
                const document = {
                    name: sanitizedName.substring(0, 100),
            ...
            if (error.code === 'LIMIT_FILE_SIZE')
                return res.status(400).json({ status: 'error', message: 'El archivo excede el tamaño máximo permitido (5MB)' });
            if (error.code === 'LIMIT_UNEXPECTED_FILE')
                return res.status(400).json({ status: 'error', message: 'Tipo de archivo no permitido o demasiados archivos' });
            
            res.status(500).json({ status: 'error', message: 'Error interno del servidor al subir documentos' });
        }
    };
    ...
    getUserDocuments = async (req, res) => {
        try {
            ...
            if (requestingUserId !== userId && req.user?.role !== 'admin')
                return res.status(403).json({ status: 'error', message: 'No autorizado para ver estos documentos' });
    ...
    deleteDocument = async (req, res) => {
        try {
            const userId = req.params.uid, documentId = req.params.did;
            
            if (!userId.match(/^[0-9a-fA-F]{24}$/) || !documentId.match(/^[0-9a-fA-F]{24}$/))
                return res.status(400).json({ status: 'error', message: 'ID de usuario o documento inválido' });
            ...
            if (req.user?.id !== userId && req.user?.role !== 'admin')
                return res.status(403).json({ status: 'error', message: 'No autorizado para eliminar documentos' });
            ...
            const result = await usersService.update(userId, { $pull: { documents: { _id: documentId } } });
            logger.info(`Documento eliminado: ${documentToDelete.name} del usuario: ${userId}`);
            
            res.json({
                status: 'success',
                message: 'Documento eliminado exitosamente',
                data: { deletedDocument: documentToDelete, remainingDocuments: result.documents.length }
            });
        } catch (error) {
    ...
    checkRequiredDocuments = async (req, res) => {
        try {
            ...
            if (req.user?.id !== userId && req.user?.role !== 'admin')
                return res.status(403).json({ status: 'error', message: 'No autorizado para verificar documentos' });
            ...
            const userDocNames = user.documents.map(doc => doc.name.toLowerCase().replace(/\.[^/.]+$/, ""));
            
            const results = {};
            for (const [type, requiredDocs] of Object.entries(requiredDocumentTypes)) {
                const missingDocuments = requiredDocs.filter(doc => !userDocNames.includes(doc));
                results[type] = {
    ...
    getDocumentById = async (req, res) => {
        try {
            ...
            if (req.user?.id !== userId && req.user?.role !== 'admin')
                return res.status(403).json({ status: 'error', message: 'No autorizado para ver este documento' });
            ...
            res.json({
                ...
                data: {
                    document,
                    user: { id: user._id, name: `${user.first_name} ${user.last_name}` }
                }
            });
        } catch (error) {
