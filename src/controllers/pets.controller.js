...
const getAllPets = async(req, res) => {
    try {
        ...
        if (!result.results || result.results.length === 0) {
            req.logger.warning('No se encontraron mascotas con los filtros aplicados');
        } else {
            req.logger.info(`Se encontraron ${result.results.length} mascotas`);
        }
    ...
    try {
        ...
        if (!name || !specie || !birthDate) {
            ...
            return res.status(400).json({ 
                status: "error", 
                error: "Valores incompletos. Se requieren: name, specie, birthDate" 
            });
        }
        ...
        res.status(201).json({ 
            ...
            data: PetDTO.getPetOutputFrom(result) 
        });
    } catch (error) {
...
const updatePet = async(req, res) => {
    try {
        ...
        const invalidFields = Object.keys(petUpdateBody).filter(
...
const deletePet = async(req, res) => {
    try {
        ...
        res.json({ 
            ...
            data: { id: petId, deletedAt: new Date() }
        });
    } catch (error) {
...
const createPetWithImage = async(req, res) => {
    try {
        ...
        res.status(201).json({ 
            status: "success", 
            message: "Mascota con imagen creada exitosamente",
            data: PetDTO.getPetOutputFrom(result)
        });
    } catch (error) {
...
const getPetById = async (req, res) => {
    try {
        ...
        res.json({
            status: "success",
            data: PetDTO.getPetOutputFrom(pet)
        });
    } catch (error) {
...
const getAvailablePets = async (req, res) => {
    try {
        ...
        res.json({
            ...
            data: petsData,
            pagination: result.pagination,
            message: petsData.length > 0 
                ? `${petsData.length} mascotas disponibles` 
