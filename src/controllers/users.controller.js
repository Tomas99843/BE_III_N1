...
const getAllUsers = async (req, res) => {
    try {
        ...
        const sanitizedUsers = result.results.map(user => 
            UserDTO.getUserProfileFrom(user)
        );
...
const getUser = async (req, res) => {
    try {
        ...
        res.json({
            status: "success",
            data: UserDTO.getUserProfileFrom(user)
        });
    } catch (error) {
...
const updateUser = async (req, res) => {
    try {
        ...
        const invalidFields = Object.keys(updateBody).filter(
        ...
        res.json({
            ...
            data: UserDTO.getBasicUserInfo(result)
        });
    } catch (error) {
...
const deleteUser = async (req, res) => {
    try {
        ...
        if (user.role === 'admin' && req.user?.role !== 'admin') {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para eliminar administradores"
            });
        }
...
const getUserDocuments = async (req, res) => {
    try {
        ...
        if (req.user?.id !== userId && req.user?.role !== 'admin') {
            return res.status(403).json({
                status: "error",
                error: "No autorizado para ver estos documentos"
            });
        }
